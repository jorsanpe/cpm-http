...
bool Endpoint::matches(const std::string& path)
{
    smatch base_match;
    return regex_match(path, base_match, this->matching_regex);
}


struct HttpParameterMap Endpoint::match(const string& path)
{
    ...
    if (regex_match(path, base_match, this->matching_regex)) {
        for (size_t i=1; i<base_match.size(); ++i) {
            ...
            match.set(this->parameter_names.at(i-1), sub_match.str());
        }
    }
